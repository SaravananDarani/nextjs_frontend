...
const {
    ...
    useForm, validator, useRouter, adminLayout, ClientInput, nextBase64, useEffect, useState
} = GlobleImport;
...
const client = () => {
    ...
    let inputs: Array<FormFieldInterface> = ClientInput;
    let company: any = '';
    if (typeof window !== 'undefined') {
        company = Constants.webid
    }
    const { clientUrl } = Constants.routerUrl;
    ...
    const handleAdd = () => {
        let dataJson = state
        const req = { data: dataJson, url: IndexServices.MasterServices.clientUrl }
    ...
    const handleUpdate = () => {
        ...
        const req = {
            data: dataJson,
            url: IndexServices.MasterServices.clientUrl
        }
    ...
    const getPosts = async () => {
        var db = {
            dataId: { id: id },
            url: `${IndexServices.MasterServices.clientUrl}/getid`,
        }
